...
One way to measure this is to define an temporal *intersection profile*
...
In empirical temporal hypergraphs, the intersection profile often $r_k^{(t)}$ decays at rates close to $t^{-1}$, *regardless of $k$*. 

This is interesting because a fully random null model of hypergraph growth would predict that $k$-intersections decay at rate $t^{-k}$. 

[Benson et al. (2018). Simplicial closure and higher-order link prediction. *PNAS*<br> Chodrow (2020). Configuration models of random hypergraphs. *JCN* <br> Landry, Young, and Eikmeier (2023). The simpliciality of higher-order networks. *EPJ Data Sci.*]{.footnote .absolute bottom=0} 
...
[Dashed line gives slope of $m^{-1}$ decay.]{.footnote}
...
**Claim** (He, Chodrow, Mucha '23): As $t$ grows large, the degrees of $H_t$ converge in distribution to a power law with exponent  
$$
...
<br> <br> <br> <br> 
**Claim**: There exist constants $q_k$ such that, as $m\rightarrow \infty$, 
...
Furthermore, the constants $\{q_k\}$ can be found by solving an eigenproblem $\mathbf{A}\mathbf{q} = \mathbf{q}$, where the matrix $\mathbf{A}$ is determined by the model parameters. 
...
![](fig/intersection-asymptotics-no-theory.png){.absolute top=100 left=50 width=500}
...
We computed AUC scores for a link prediction task in which our model learns parameters on 20% training data from three metabolic and organic reaction data sets. We compared these against previously-reported AUC scores for two dedicated **neural** network hypergraph link prediction algorithms. 

[Yadati et al. (2020) NHP: Neural Hypergraph Link Prediction. *CIKM* <br> 
