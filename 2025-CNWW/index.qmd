...
subtitle: "Complex Networks Winter Workshop | Dec 15th, 2025"  
date: ""
...
I'm a new-ish professor in the department of computer science at Middlebury College in Middlebury, Vermont. 
...
#### Others are...different. [ttttttt ttttttt ttttt]{.hide}
...
[![](img/mari_portrait.jpeg) <br> Mari Kawakatsu <br> UPenn, UMich]{.absolute width=160 left=10}
...
### Ranking and Networks

::: {.column}

<br> <br> <br> 

For today, we're going to consider a *weighted directed network* on $n$ agents, which we represent via a (not-necessarily symmetric) weighted adjacency matrix $\mathbf{A} \in \mathbb{R}^{n\times n}$.

A ***ranking*** is a function that assigns a score to each agent. We can represent a ranking as a function which assigns to matrix $\mathbf{A}$ a vector $\mathbf{r} = \mathbf{r}(\mathbf{A})$. 

Here, $\mathbf{r} = (r_1,r_2,\ldots, r_n)$ where $r_i$ is the score of agent $i$.
...
![](img/springrank-viz.jpeg){.absolute width=150 left=100}

<br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> 

[*Image credit*: De Bacco, Larremore + Moore, *Science Advances* (2018)]{.footnote}

:::

## 

### From networks to ranking[...and back?]{.fragment fragment-index="2"}

<!-- We often ask: given a network, what is the "best" ranking of the nodes? 

- PageRank, SpringRank, etc. 
- Elo ratings
...
n_agents = 10
A = np.random.rand(n_agents, n_agents) > 0.7
A[np.eye(n_agents, dtype=bool)] = 0  # No self-loops

G = nx.from_numpy_array(A.astype(int), create_using=nx.DiGraph)

# Plot the original graph
fig, ax = plt.subplots(1, 2, figsize=(12, 6), width_ratios=[1, 0.4])
pos = nx.spring_layout(G)
nx.draw(G, pos, node_color='lightcoral', 
  node_size=500, arrowsize=20, arrowstyle='->', ax = ax[0], edgecolors = "white", edge_color='grey')

# Compute centrality scores
centrality = nx.pagerank(G)
ranks = np.array([centrality[i] for i in range(len(G))])

# Plot with node positions based on ranks
plt.subplot(1, 2, 2)
pos_ranks = {i: (0, ranks[i]) for i in G.nodes()}

nx.draw_networkx_nodes(G, pos_ranks, node_color='lightcoral', node_size=500, ax = ax[1], edgecolors='white')
nx.draw_networkx_edges(G, pos_ranks, arrowsize=20, arrowstyle='-', ax = ax[1], edge_color = "grey")
...
plt.savefig("img/ranking-viz.png")

``` -->

![](img/ranking-viz.png){.absolute width=1000 left=50 top=100}
![](img/ranking-viz-one-arrow.png){.absolute width=1000 left=50 top=100 .fragment fragment-index="1"}
![](img/ranking-viz-two-arrows.png){.absolute width=1000 left=50 top=100 .fragment fragment-index="2"}
...
When agent $i$ endorses agent $j$, we interpret this as agent $i$ signaling that agent $j$ is of high prestige. 
...
Here, our idea is that the updates $\Delta^{(t)}$ are sensitive to agents' current perceptions of ranks in the network $\mathbf{A}^{(t)}$. 
...
$$
u_{ij}(\mathbf{r}) = \beta_1 r_j + \beta_2 (r_i - r_j)^2. 
$$

Agent $i$ endorses $j$ at time $t+1$ w. p. 
$$
p_{ij}^{(t)} = \frac{e^{u_{ij}(\mathbf{r}^{(t)})}}{\sum_j e^{u_{ij}(\mathbf{r}^{(t)})}}
$$

($m\in \Z$ endorsements per round.)
...
Each agent $i$ computes a *utility* of endorsing agent $j$ in terms of the score vector $\mathbf{r}^{(t)}$. Our utility is: 
$$
u_{ij}(\mathbf{r}) = \color{#FFD447}{\beta_1} r_j + \color{#59C9A5}{\beta_2} (r_i - r_j)^2. 
$$
...
[Yes, and there is a bifurcation (qualitative behavior shift) depending on the parameters.]{.fragment}
...
##### It matters how we compute the ranks! <br>
...
[We have reason to prefer Root-Degree here -- it's a better predictor of observed behaviors!]{.fragment} 
...
We developed a simple [math model]{.alert} of hierarchies emerging from feedback loops.
...
![](img/plagiarized.png){.fragment fragment-index="1"}
![](img/plagiarized-2.png){.fragment fragment-index="2" .absolute left=50 top=150}
![](img/plagiarized-3.png){.fragment fragment-index="3" .absolute left=75 top=150}
...
[Yang, X.; Wu, Z.; Javaid, S. RETRACTED: An Adaptive Hierarchical Network Model for Studying the Structure of Economic Network. *Entropy* 2022, 24, 702.]{.absolute left=10 top=200 width=250 .fragment fragment-index="1"}
...
Communicate with them honestly and often. 
